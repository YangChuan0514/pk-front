<template>
  <swiper :slides-per-view="1" :space-between="0" @swiper="onSwiper" @slideChange="onSlideChange"
    :class="getClassAndStyle(props.height).class" :style="getClassAndStyle(props.height).style" :modules="modules"
    :navigation="{ prevEl: '.prev', nextEl: '.next' }" :pagination="{ type: 'fraction', el: '.pagination' }"
    v-bind="$attrs">
    ...
    <div
      ...
      <div class="cursor-pointer prev" @click.stop="onPrev">
        <i-mdi-arrow-left-thin style="font-size: 2rem" />
      </div>
      <div class="cursor-pointer next" @click.stop="onNext">
        <i-mdi-arrow-right-thin style="font-size: 2rem" />
      </div>
    </div>
  </swiper>
</template>
...
const onPrev = () => {
  console.log('onPrev', swiperIns.value)
  if (swiperIns.value) {
    if (swiperIns.value.isBeginning) return
    swiperIns.value.slidePrev(1000)
    return
  }
}
const onNext = () => {
  console.log('onNext', swiperIns.value)
  if (swiperIns.value) {
    if (swiperIns.value.isEnd) return
    swiperIns.value.slideNext(1000)
    return
  }
}
</script>
